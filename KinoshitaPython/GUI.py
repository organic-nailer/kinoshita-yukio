...
Yukio = Kinoshita('Yukio')     # Yukioオブジェクトを保持
...
def prompt():
    ...
    p = Yukio.name
    if (action.get())==0:
        p += '：' + Yukio.responder.name
    return p + '> '


def talk():
    """ 対話を行う関数
        ・YukioクラスのDialogue()を実行して応答メッセージを取得
...
def writelog():
    ...
    now = "Kinoshita_" + datetime.now().strftime("%Y-%m-%d %H:%m::%S" + "\n")
